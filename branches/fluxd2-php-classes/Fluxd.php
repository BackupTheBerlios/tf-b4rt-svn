...
class Fluxd
{
    ...
    function Fluxd() {
        ...
        $this->_pathLogFileError = $this->_pathDataDir . 'fluxd.log';
    ...
    function instance_start() {
        ...
        if ($this->state == FLUXD_STATE_RUNNING) {
			...
			if (@file_exists($cfg['pythonCmd']) !== true) {
				$msg = "cannot start fluxd, specified Python-binary does not exist: ".$cfg['pythonCmd'];
            ...
            $startCommand .= " --file.php=".tfb_shellencode($cfg["bin_php"]);
            $startCommand .= " --database.DataAdapter='DataAdapterFluxcli'";
