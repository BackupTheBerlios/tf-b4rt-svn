...
class ClientHandlerAzureus extends ClientHandler
{
    ...
    function start($transfer, $interactive = false, $enqueue = false) {
		...
		#$this->command  = "echo -e ".tfb_shellencode($content)." > ".tfb_shellencode($cfg["path"].'.fluxd/fluazu/run/'.$transfer);
		#$this->command .= " && ";
		$handle = false;
		$commandFile = $cfg["path"].'.fluxd/fluazu/run/'.$transfer;
		$handle = @fopen($commandFile, "w");
		if (!$handle) {
            $msg = "cannot open command-file ".$commandFile." for writing.";
            array_push($this->messages , $msg);
			return false;
		}
		...
		@fclose($handle);
		if ($result === false) {
            $msg = "cannot write content to command-file ".$commandFile.".";
            array_push($this->messages , $msg);
			return false;
		}
		
		// build the command-string
		$this->command = "echo r > ".tfb_shellencode($cfg["path"].'.fluxd/fluazu/fluazu.cmd');
