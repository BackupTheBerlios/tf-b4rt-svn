...
class MaintenanceAndRepair
{
	...
	function _maintenanceFluxd() {
		...
		if ($fctr > 0) {
			...
			*/
			/* fluxd2 */
			if ("1" != @trim(shell_exec("ps x -o pid='' -o ppid='' -o command='' -ww 2> /dev/null | ".$cfg['bin_grep']." -v grep | ".$cfg['bin_grep']." fluxd.py | ".$cfg['bin_grep']." -c ".tfb_shellencode($cfg["docroot"], 0 , strlen($cfg["docroot"]) - 1))))
