...
# fluazu
from fluxd.modules.Modules.fluazu.TransferFile import TransferFile
################################################################################
...
class Transfer(object):
    ...
    """ __init__                                                             """
    """ -------------------------------------------------------------------- """
    def __init__(self, logger, tf_pathTransfers, flu_pathTransfers, file):
    ...
    def initialize(self):
        ...
        # out
        self.logger.info("initializing transfer %s ..." % self.name)

        # meta-file
        self.logger.info("loading transfer-file %s ..." % self.fileMeta)
        self.tf = TransferFile(self.logger, self.fileMeta)

        # stat-file
        self.logger.info("loading statfile %s ..." % self.fileStat)
        self.sf = StatFile(self.fileStat)

        # verbose
        self.logger.info("transfer loaded.")
    ...
    def start(self, download):
        ...
        try:
            ...
            self.log("exception when starting transfer (%s)" % e)
    ...
    def stop(self, download):
        ...
        try:
            ...
            self.log("exception when stopping transfer (%s)" % e)
            retVal = False
    ...
    def log(self, message):
        ...
        try:
            ...
            f.write(self.getOutput(message))
            f.flush()
            f.close()
        except Exception, e:
            self.logger.error("Failed to write log-file %s (%s)" % (self.fileLog , e))
